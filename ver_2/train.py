...
FILENAME = 'mnist_dnn_784_50_40_30_20_10.ps'
...
m = MLP.DNN('784x50x40x30x20x10', 0.00001, 0.25)
m.load_net(FILENAME)
...
while True:
    ...
    print(f'\nTrained on 60,000 images with accuracy {train_correct / 60_000}, now testing...\n')
    
    for q in tqdm(range(10_000), ncols = 80,desc='Testing net'):
        if classify_result(m.feed_forward(np.reshape(test_X[q].flatten() * (1/255), (-1, 1)))) == test_y[q]:
            correct = correct + 1
